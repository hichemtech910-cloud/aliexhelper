...
def detect_category(title):
    t = title.lower()
    if any(kw in t for kw in ['phone', 'case', 'cover', 'funda', 'screen protector', 'iphon', 'samsung', 'xiaomi']):
        return '📱 Phones'
    if any(kw in t for kw in ['earbuds', 'headphone', 'speaker', 'earphone', 'airpod']):
        return '🎧 Audio'
    if any(kw in t for kw in ['watch', 'band', 'fitness', 'smartwatch']):
        return '⌚ Watches'
    if any(kw in t for kw in ['cable', 'charger', 'power bank', 'usb', 'adapter']):
        return '🔌 Electronics'
    if any(kw in t for kw in ['kitchen', 'cooking', 'pot', 'pan', 'blender']):
        return '🍳 Kitchen'
    if any(kw in t for kw in ['car', 'vehicle', 'auto']):
        return '🚗 Cars'
    if any(kw in t for kw in ['gaming', 'controller', 'mouse', 'keyboard', 'gamepad']):
        return '🎮 Gaming'
    if any(kw in t for kw in ['dress', 'shirt', 'pants', 'shoes', 'bag', 'wallet', 'jacket', 'hat', 'sunglasses', 'fashion']):
        return '👗 Fashion'
    return '📦 Other'
